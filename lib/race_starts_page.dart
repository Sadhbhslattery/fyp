...
class _RaceStartsPageState extends State<RaceStartsPage> {
  ...
  // ------------------------------
  // FINISH A BOAT (with OCS + Penalty options)
  // ------------------------------
  //
  // This replaces the old _finishBoat() method.
  // When the race officer taps "Finish now", we show a bottom sheet that allows:
  ...
  Future<void> _finishBoatWithOptions(Map<String, dynamic> boat) async {
    ...
    final confirmed = await showModalBottomSheet<bool>(
      ...
      builder: (context) {
        return Padding(
          // Add padding + account for keyboard so fields aren’t hidden
          ...
          child: Column(
            ...
            children: [
              ...
              // We use StatefulBuilder so the switch can update inside the sheet
              StatefulBuilder(
    ...
    try {
      // POST finish + options to backend
      // IMPORTANT: These keys MUST match what your FastAPI endpoint expects.
      ...
      // Reload the boats + timings so UI updates
      await _loadData();
