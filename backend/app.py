"""
app.py  FastAPI backend for Regatta Timing App (RCYC)
...
This file contains:
  ...
  • Static course definitions (Course 1 - 3)
  • Boat CRUD endpoints (for fleet admin page)
...
import csv # Python CSV writer (standard library)
...
# - Depends: dependency injection (for DB session)
# - HTTPException: for returning proper HTTP error responses
# Reference: FastAPI basic app, middleware & CORS setup [B1]

from fastapi.middleware.cors import CORSMiddleware
# CORSMiddleware allows the Flutter app on another origin (localhost:XXXXX)
# to call this API without browser CORS issues (especially for web builds)

from pydantic import BaseModel, field_validator
# BaseModel: defines request/response schemas (data validation)
# field_validator: Pydantic v2 decorator to validate/transform fields
...
# Base: SQLAlchemy base class for models (metadata)
